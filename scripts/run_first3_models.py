...
from luc_forgery_pipeline.config import KERAS_MODEL_NAMES, ExperimentConfig
from luc_forgery_pipeline.train import train_one_model

FIRST3_MODELS = ("plain_unet", "efficientnetb0_unet", "resnet50_unet")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train the first three LUC forgery segmentation baselines.")
    ...
    parser.add_argument("--models", nargs="+", default=list(FIRST3_MODELS), choices=list(KERAS_MODEL_NAMES))
    parser.add_argument("--samples-per-class", type=int, default=300)
    parser.add_argument("--full-data", action="store_true", help="Use all available train images instead of the balanced subset.")
