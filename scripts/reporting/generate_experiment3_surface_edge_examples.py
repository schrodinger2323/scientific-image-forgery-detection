...
def ensure_package(module_name: str, pip_name: str) -> None:
    try:
        ...
        print(f"[install] {module_name} not found; installing {pip_name} ...")
    try:
        subprocess.check_call([sys.executable, "-m", "pip", "install", "-q", pip_name])
    except subprocess.CalledProcessError as exc:  # pragma: no cover - environment guard
        raise SystemExit(
            f"Could not install {pip_name}. In Kaggle, enable Internet or run this cell first:\n"
            f"!pip install -q {pip_name} timm\n"
        ) from exc
